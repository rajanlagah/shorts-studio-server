...
export const edit=z.object({clips:z.array(z.object({assetId:uuid,start:z.number().min(0).max(3600),end:z.number().positive().max(3600),fit:z.enum(['fit','crop']).default('fit')}).strict().refine(c=>c.end>c.start)).min(1).max(5),captions:z.array(caption).max(500).default([])}).strict().superRefine((p,ctx)=>{
