...
const {pool,assetPath}=await import('../src/common.js');
const session=randomUUID(),resolve=id=>assetPath(session,id),a=randomUUID(),b=randomUUID(),folder=join(temp,session);await mkdir(folder);
try{
 ...
 await exportVideo(resolve,payload,work,{});
 ...
 assert.ok(!ass([{start:0,end:1,text:'{\\pos(1,1)}'}],LEGACY.classic).includes('{\\pos(1'));
 await assert.rejects(exportVideo(resolve,{clips:[{assetId:a,start:0,end:8}],captions:[]},join(folder,randomUUID()),{}),/Trim exceeds/);
